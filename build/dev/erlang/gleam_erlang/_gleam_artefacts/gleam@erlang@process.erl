...
-opaque subject(ADC) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, ADC}.

-type do_not_leak() :: any().

-type selector(ADD) :: any() | {gleam_phantom, ADD}.
...
-type call_error(ADE) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, ADE}.
...
-spec select(selector(ADV), integer()) -> {ok, ADV} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-spec select_forever(selector(ADZ)) -> ADZ.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-spec map_selector(selector(AEB), fun((AEB) -> AED)) -> selector(AED).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-spec merge_selector(selector(AEF), selector(AEF)) -> selector(AEF).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(AEJ), fun((exit_message()) -> AEJ)) -> selector(AEJ).
...
-spec selecting(selector(AEM), subject(AEO), fun((AEO) -> AEM)) -> selector(AEM).
...
-spec 'receive'(subject(ADP), integer()) -> {ok, ADP} | {error, nil}.
...
-spec selecting_record2(
    selector(AER),
    any(),
    fun((gleam@dynamic:dynamic_()) -> AER)
) -> selector(AER).
...
-spec selecting_record3(
    selector(AEV),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> AEV)
) -> selector(AEV).
...
-spec selecting_record4(
    selector(AEZ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> AEZ)
) -> selector(AEZ).
...
-spec selecting_record5(
    selector(AFD),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> AFD)
) -> selector(AFD).
...
-spec selecting_record6(
    selector(AFH),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> AFH)
) -> selector(AFH).
...
-spec selecting_record7(
    selector(AFL),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> AFL)
) -> selector(AFL).
...
-spec selecting_record8(
    selector(AFP),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> AFP)
) -> selector(AFP).
...
-spec selecting_anything(selector(AFT), fun((gleam@dynamic:dynamic_()) -> AFT)) -> selector(AFT).
...
-spec try_call(subject(AGE), fun((subject(AGG)) -> AGE), integer()) -> {ok, AGG} |
    {error, call_error(AGG)}.
...
-spec call(subject(AGL), fun((subject(AGN)) -> AGL), integer()) -> AGN.
...
-spec send_after(subject(AGQ), integer(), AGQ) -> timer().
