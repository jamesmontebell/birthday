...
-spec set_header(request(BEF), binary(), binary()) -> request(BEF).
...
-spec set_query(request(BFB), list({binary(), binary()})) -> request(BFB).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(BFF), gleam@http:method()) -> request(BFF).
...
-spec set_scheme(request(BFM), gleam@http:scheme()) -> request(BFM).
set_scheme(Req, Scheme) ->
    erlang:setelement(5, Req, Scheme).

-spec set_host(request(BFP), binary()) -> request(BFP).
set_host(Req, Host) ->
    erlang:setelement(6, Req, Host).

-spec set_port(request(BFS), integer()) -> request(BFS).
set_port(Req, Port) ->
    erlang:setelement(7, Req, {some, Port}).

-spec set_path(request(BFV), binary()) -> request(BFV).
set_path(Req, Path) ->
    erlang:setelement(8, Req, Path).

-spec set_cookie(request(BFY), binary(), binary()) -> request(BFY).
