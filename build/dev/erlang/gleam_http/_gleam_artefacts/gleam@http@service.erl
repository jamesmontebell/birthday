...
-spec map_response_body(
    fun((BPM) -> gleam@http@response:response(BOY)),
    fun((BOY) -> BOZ)
) -> fun((BPM) -> gleam@http@response:response(BOZ)).
...
-spec prepend_response_header(
    fun((BPQ) -> gleam@http@response:response(BPS)),
    binary(),
    binary()
) -> fun((BPQ) -> gleam@http@response:response(BPS)).
...
-spec ensure_post(gleam@http@request:request(BPD)) -> {ok,
        gleam@http@request:request(BPD)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(BQZ)) -> BRH)) -> fun((gleam@http@request:request(BQZ)) -> BRH).
