...
-type message(BRW) :: {message, BRW} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(BRX, BRY) :: {continue,
        BRY,
        gleam@option:option(gleam@erlang@process:selector(BRX))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(BRZ, BSA) :: {ready, BRZ, gleam@erlang@process:selector(BSA)} |
    {failed, binary()}.

-type self(BSB, BSC) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        BSB,
        gleam@erlang@process:subject(BSC),
        gleam@erlang@process:selector(message(BSC)),
        gleam@otp@system:debug_state(),
        fun((BSC, BSB) -> next(BSC, BSB))}.

-type spec(BSD, BSE) :: {spec,
        fun(() -> init_result(BSD, BSE)),
        integer(),
        fun((BSE, BSD) -> next(BSE, BSD))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(BSF) :: {ack,
        {ok, gleam@erlang@process:subject(BSF)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-spec continue(BSM) -> next(any(), BSM).
continue(State) ->
    {continue, State, none}.

-spec with_selector(next(BSQ, BSR), gleam@erlang@process:selector(BSQ)) -> next(BSQ, BSR).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(BTC))) -> gleam@erlang@process:selector(message(BTC)).
...
-spec receive_message(self(any(), BSY)) -> message(BSY).
...
-spec init_selector(
    gleam@erlang@process:subject(BXQ),
    gleam@erlang@process:selector(BXQ)
) -> gleam@erlang@process:selector(message(BXQ)).
...
-spec initialise_actor(
    spec(any(), BTT),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(BTT)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), BUG)) -> {ok, gleam@erlang@process:subject(BUG)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(BUM, fun((BUN, BUM) -> next(BUN, BUM))) -> {ok,
        gleam@erlang@process:subject(BUN)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(BUT), BUT) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-spec call(
    gleam@erlang@process:subject(BUV),
    fun((gleam@erlang@process:subject(BUX)) -> BUV),
    integer()
) -> BUX.
