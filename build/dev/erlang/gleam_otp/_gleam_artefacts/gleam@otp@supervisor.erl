...
-type spec(CDR, CDS) :: {spec,
        CDR,
        integer(),
        integer(),
        fun((children(CDR)) -> children(CDS))}.

-opaque children(CDT) :: {ready, starter(CDT)} | {failed, child_start_error()}.

-opaque child_spec(CDU, CDV, CDW) :: {child_spec,
        fun((CDV) -> {ok, gleam@erlang@process:subject(CDU)} |
            {error, gleam@otp@actor:start_error()}),
        fun((CDV, gleam@erlang@process:subject(CDU)) -> CDW)}.
...
-spec start_child(child_spec(any(), CED, CEE), CED) -> {ok, child(CEE)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    CER,
    instruction(),
    child_spec(any(), CER, CET),
    child(CET)
) -> {ok, {child(CET), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(CFK), child_spec(any(), CFK, CFN)) -> children(CFN).
...
-spec add(children(CFS), child_spec(any(), CFS, CFV)) -> children(CFV).
...
-spec supervisor(
    fun((CGA) -> {ok, gleam@erlang@process:subject(CGB)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(CGB, CGA, CGA).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec worker(
    fun((CGI) -> {ok, gleam@erlang@process:subject(CGJ)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(CGJ, CGI, CGI).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec returning(
    child_spec(CGQ, CGR, any()),
    fun((CGR, gleam@erlang@process:subject(CGQ)) -> CGX)
) -> child_spec(CGQ, CGR, CGX).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-spec init(spec(any(), CHC)) -> gleam@otp@actor:init_result(state(CHC), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(CHI)) -> gleam@otp@actor:next(message(), state(CHI)).
...
-spec loop(message(), state(CHN)) -> gleam@otp@actor:next(message(), state(CHN)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
