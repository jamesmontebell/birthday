...
-opaque task(CQN) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(CQN))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(CQO) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, CQO}.

-spec async(fun(() -> CQP)) -> task(CQP).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(CQT), integer()) -> {ok, CQT} | {error, await_error()}.
...
-spec await(task(CQX), integer()) -> CQX.
await(Task, Timeout) ->
...
-spec try_await_forever(task(CQZ)) -> {ok, CQZ} | {error, await_error()}.
...
-spec await_forever(task(CRD)) -> CRD.
await_forever(Task) ->
