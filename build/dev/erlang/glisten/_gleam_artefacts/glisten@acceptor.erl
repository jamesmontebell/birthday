...
-type pool(DLU, DLV) :: {pool,
        glisten@socket:listen_socket(),
        fun((glisten@handler:loop_message(DLU), DLV, glisten@handler:connection(DLU)) -> gleam@otp@actor:next(glisten@handler:loop_message(DLU), DLV)),
        integer(),
        fun(() -> {DLV, gleam@option:option(gleam@erlang@process:selector(DLU))}),
        gleam@option:option(fun((DLV) -> nil)),
        glisten@socket@transport:transport()}.
