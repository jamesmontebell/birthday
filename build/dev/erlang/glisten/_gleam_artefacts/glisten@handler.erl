...
-type message(DCY) :: {internal, internal_message()} | {user, DCY}.

-type loop_message(DCZ) :: {packet, bitstring()} | {custom, DCZ}.

-type loop_state(DDA, DDB) :: {loop_state,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(DDA)),
        glisten@socket@transport:transport(),
        DDB}.

-type connection(DDC) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@socket@transport:transport(),
        gleam@erlang@process:subject(message(DDC))}.

-type handler(DDD, DDE) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(DDD), DDE, connection(DDD)) -> gleam@otp@actor:next(loop_message(DDD), DDE)),
        fun(() -> {DDE, gleam@option:option(gleam@erlang@process:selector(DDD))}),
        gleam@option:option(fun((DDE) -> nil)),
        glisten@socket@transport:transport()}.

-spec start(handler(DDO, any())) -> {ok,
        gleam@erlang@process:subject(message(DDO))} |
