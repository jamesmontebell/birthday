...
-opaque builder(FIJ, FIK) :: {builder,
        integer(),
        fun((gleam@http@request:request(FIJ)) -> gleam@http@response:response(FIK)),
        fun((integer(), gleam@http:scheme()) -> nil)}.

-type websocket_message(FIL) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, FIL}.
...
-spec new(
    fun((gleam@http@request:request(FJG)) -> gleam@http@response:response(FJI))
) -> builder(FJG, FJI).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(FJM, FJN), integer()) -> builder(FJM, FJN).
port(Builder, Port) ->
    erlang:setelement(2, Builder, Port).

-spec read_request_body(
    builder(bitstring(), FJS),
    integer(),
    gleam@http@response:response(FJS)
) -> builder(mist@internal@http:connection(), FJS).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(FKQ)
) -> {ok, websocket_message(FKQ)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((FKW, mist@internal@websocket:websocket_connection(), websocket_message(FKX)) -> gleam@otp@actor:next(FKX, FKW)),
    fun((mist@internal@websocket:websocket_connection()) -> {FKW,
        gleam@option:option(gleam@erlang@process:selector(FKX))}),
    fun((FKW) -> nil)
