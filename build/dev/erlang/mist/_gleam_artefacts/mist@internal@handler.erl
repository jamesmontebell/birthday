...
-spec close_or_set_timer(
    gleam@http@response:response(response_data()),
    mist@internal@http:connection(),
    gleam@erlang@process:subject(glisten@handler:message(ENM))
) -> gleam@otp@actor:next(glisten:message(ENM), state()).
...
) -> fun((glisten:message(ENE), state(), glisten:connection(ENE)) -> gleam@otp@actor:next(glisten:message(ENE), state())).
with_func(Handler) ->
    fun(Msg, State, Conn) ->
