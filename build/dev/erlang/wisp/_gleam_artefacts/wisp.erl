...
-spec method_override(gleam@http@request:request(CJU)) -> gleam@http@request:request(CJU).
...
-spec fn_with_bad_request_error(fun((CKV) -> {ok, CKW} | {error, any()})) -> fun((CKV) -> {ok,
        CKW} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((CKP, bitstring()) -> {ok, CKP} |
        {error, gleam@http@response:response(body())}),
    CKP
) -> {ok, {gleam@option:option(buffered_reader()), integer(), CKP}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), CLN})) -> list({binary(), CLN}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, CLQ} | {error, any()},
    fun((CLQ) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, CKH} | {error, any()}) -> {ok, CKH} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
