...
-type valid_message(KLI) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, KLI}.

-type websocket_message(KLJ) :: {valid, valid_message(KLJ)} | invalid.

-type websocket_connection() :: {websocket_connection,
        glisten@socket:socket(),
        glisten@socket@transport:transport()}.

-type handler_message(KLK) :: {internal, frame()} | {user, KLK}.

-type websocket_state(KLL) :: {websocket_state, bitstring(), KLL}.
...
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(KNA))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(KNA))).
...
-spec apply_frames(
    list(frame()),
    fun((KMK, websocket_connection(), handler_message(KML)) -> gleam@otp@actor:next(KML, KMK)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(KML), KMK),
    fun((KMK) -> nil)
) -> gleam@otp@actor:next(websocket_message(KML), KMK).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {KLX,
        gleam@option:option(gleam@erlang@process:selector(KLY))}),
    fun((KLX) -> nil),
    fun((KLX, websocket_connection(), handler_message(KLY)) -> gleam@otp@actor:next(KLY, KLX)),
    glisten@socket:socket(),
    glisten@socket@transport:transport()
) -> {ok, gleam@erlang@process:subject(websocket_message(KLY))} | {error, nil}.
